...
- 目的に応じてチャットボットを作成することができます。(プロンプトやモデル等)
- ログインユーザ毎に、自分のチャットボットを作成できます。(自分専用 or 他のユーザに公開)
- 会話履歴は自動的にデータベースに保存されます。
...
- 本システムのパスワード管理は簡易的なものです。パスワードなど情報漏洩には気をつけて下さい。
- 多人数の同時アクセスにに耐えるシステムではありません。
- 本システムはOpenAIのAPIを使用します。APIの利用にはOpenAIの利用規約が適用されます。
- 本システムの使用によって生じるいかなる損害についても、開発者は責任を負いません。
- 本システムは、開発段階のものです。予告なく機能の変更などが発生する可能性があります。

## 🦀データ保存先

ユーザ設定、Bot設定、会話履歴は、すべてChromaDBのデータとして保存されます。

保存先ディレクトリ:
  - Linux: $HOME/.data/crabAI
  - Windows: C:\Users\ユーザ名\.data\crabAI

不要になったら、ディレクトリごと全部削除して下さい。
...
## 🦀ToDo
  やりたい事は色々あるが、このシステムの需要がないからおそらく実装しない。。。要望があればがんばるかも。
  - RAGをPDFに対応させる
  - 会話スレッドの削除、検索
  - 画面が使いにくい。Streamlitから他のUIに変更する？
  - 使用トークン数記録、月別、ユーザ別に集計する。
  - 会話履歴の要約機能
  - 音声会話
  - Web検索
  - コード実行機能
